...
上一节 [[18. 类的继承、访问说明符、final#5. final 说明符|18-5. final 说明符]] 中我们只学习了第一种用途，这一节我们继续学习第二种用途。
...
基类指针/引用指向派生类对象时，只能通过该指针/引用调用基类的虚函数，而不能调用派生类的虚函数：
...
int main() {
	B b;
	A* aptr = &b;
	aptr->Func1(); // ✅ 正确，只能通过 aptr 调用 A 的虚函数 Func1
	aptr->Func2(); // ❌ 错误，不能通过 aptr 调用 B 的虚函数 Func2
}
```
...
在开始下一节 [[20. 函数指针，对象内存模型，动态多态原理]] 的学习前，请先学习支线课程 [[LLaa1. 内存的深入理解]]，了解“**段**”这个内存模型。
